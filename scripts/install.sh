...
if [ ! -f "$ZIP_PATH" ]; then
  echo "==> build/polepole.zip not found. Running build first..."
  "$SCRIPT_DIR/build.sh"
fi
...
rm -rf /Applications/PolePole.app
mv "$STAGE/PolePole.app" /Applications/
rm -rf "$STAGE"

echo "==> Done: /Applications/PolePole.app"
