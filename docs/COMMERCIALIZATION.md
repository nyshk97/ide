...
価格は **国内 ¥9,900 / 海外 $79 の買い切り / Lifetime License**（全メジャーバージョン無料アップデート）で確定。¥9,900 と $79 は同一価格の為替換算ではなく **地域別価格**（国内は親しみやすい円価格、海外は価格感度の低い市場向けの強気アンカー）として扱う。サブスク前提のサポート負荷は個人開発では持続しない想定。ターゲットは AI コーディング CLI（Claude Code Pro/Max など、$20〜$200/mo）に課金している層なので、買い切り一括は許容範囲内。Lifetime にした以上、収益は新規購入のみなので価格を低めに振らない。**ローンチ時のみ 50%OFF クーポン**（後述）で初速だけ作り、定価のアンカーは崩さない（恒久値下げはしない）。
...
**確定済み:**
- **価格**: **国内 ¥9,900 / 海外 $79 の買い切り / Lifetime License**（全メジャーバージョン無料アップデート、アップグレード料なし）。¥9,900 と $79 は地域別価格（為替換算ではない）
- **ローンチ割引**: launch 時のみ 50%OFF クーポン（¥9,900 → ¥4,950）。期間 or 本数限定で初速だけ作り、恒久値下げはしない
...
**Stage 別の運用方針**:
- **Stage1 (国内・円建て)**: **Stripe**。¥9,900 の円表記で JCB 含めて受ける。MoR が要らないので税務はインボイス制度のみ自前
...
- **手数料 +1.4%**: ¥9,900 取引で Stripe `3.6%+¥40` ≈ ¥396、Paddle `5%+$0.50` ≈ ¥570。差 +¥174/件が全件に乗る
...
| 項目 | 内容 |
|---|---|
| 表示価格 | **¥9,900**（国内ローンチ価格。海外向け $79 とは地域別価格で、為替換算ではない） |
| ローンチ割引 | **launch 時のみ 50%OFF クーポン**（¥9,900 → ¥4,950）。期間 or 本数限定で初速・拡散を作り、定価 ¥9,900 のアンカーは崩さない（恒久値下げはしない）。Stripe の Promotion Code で実装（自分用無料クーポン `POLEPOLE-ETERNAL-FREE-D0NE1S` と同じ仕組み）。実際のクーポン作成は launch 直前タスク |
...
- ~~買い切り金額~~: **国内 ¥9,900 / 海外 $79（Lifetime License、地域別価格）**で確定
- ~~ローンチ割引~~: **launch 時のみ 50%OFF クーポン（¥9,900 → ¥4,950）、恒久値下げはしない**で確定
