...
| ローンチ割引 | **launch 時のみ 50%OFF クーポン**（¥9,900 → ¥4,950）。期間 or 本数限定で初速・拡散を作り、定価 ¥9,900 のアンカーは崩さない（恒久値下げはしない）。Stripe の Promotion Code で実装（自分用無料クーポンと同じ仕組み。コード自体は非公開）。実際のクーポン作成は launch 直前タスク |
