...
- [x] AI 種別バッジ: `ghostty_surface_foreground_pid` を定期取得 + `proc_pidpath` で実行ファイル名取得 + 拡張子除去で `claude` / `codex` を識別（claude のバイナリは `claude.exe` で来るので拡張子比較が必須だった）
- [x] foreground プロセス変化を監視するタイマー（500ms）→ `GhosttyManager.startForegroundPolling` + `MainActor` 隔離の `tickForegroundPolling`
