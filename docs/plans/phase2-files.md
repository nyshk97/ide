...
### 9. プレビュー履歴ナビ（半日）
- [x] 戻る/進む履歴を中央ペインのツールバーに ← → ボタンで実装
- [x] ファイルツリーで別ファイルを開くと履歴に追加
- [x] 同じファイルを連続で開いても重複しない
- [x] **動作確認**: 履歴 1 件で disable、複数開いた場合の遷移は手動

### 10. Cmd+P クイック検索（1〜2日）
- [x] アクティブプロジェクト初回オープン時にファイル+ディレクトリ一覧をインデックス
- [ ] ~~watcher で増減反映~~（Phase 2.5 へ、FSEvents 統合と一緒に）
- [x] ファジーマッチ（自前実装、隣接・先頭・全文字順ボーナス）
- [x] スラッシュ含むとパスマッチに自動切替
- [x] スコアリング: 直近開いたもの（recents）を上位
- [x] UI: 中央オーバーレイ、↑↓選択、Enter で開く、Esc キャンセル
- [ ] ~~Cmd+C でパスコピー~~（後段、cmd+c はテキストフィールド側で取られるため別ショートカットで）
- [ ] ~~「ignored を含む」トグル~~（後段）
