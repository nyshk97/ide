...
- [ ] 展開/折り畳み状態をプロジェクトごとに保持（再起動時はリセット、要件通り）
- [ ] 拡張子別アイコン（標準セット）
- [ ] `.gitignore` 対象は薄表示、トグルで完全非表示
- [ ] シンボリックリンク扱い（ディレクトリは辿らず、ファイルは表示、外部参照は明示）
- [ ] 右クリック: 相対パスコピー / ターミナルで開く（右ペイン active のターミナルで `cd` 発行）
- [ ] **動作確認**: 自分のリポジトリを開いて構造が見える、トグル切替が効く
