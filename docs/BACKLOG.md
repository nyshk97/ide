...
| ペイン間のタブ移動 (D&D + `Cmd+Shift+Opt+↑/↓`) | 2026-05-27 に一度実装したが、`addSubview` で CAMetalLayer の Metal context が新階層に再 attach されないため、移動後のタブが真っ黒になるバグで撤退（[2026-05-27-pane-layout-and-cross-pane-tabs.md](./plans/2026-05-27-pane-layout-and-cross-pane-tabs.md) Phase 3 参照）。再着手時は **Ghostty / cmux の render 経路を読み込んでから**。「2 ペイン作業中に『このタブは反対側でやりたい』と頻繁に思うようになったら」優先度を上げる |
