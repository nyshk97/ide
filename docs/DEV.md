...
| `POLEPOLE_TEST_EVENT_FILE=<path>` | **キー入力・フォーカス移動の注入フック**。`<path>`（JSONL）への追記を監視し 1 行 1 コマンドで処理する。`{"type":"key","keyCode":36,"mods":["cmd"]}`（1 キー。`chars` 省略時は keyCode から補完）/ `{"type":"text","text":"echo hi"}`（ASCII を 1 文字ずつ）/ `{"type":"focus","target":"terminal"\|"preview"\|"find"}`（first responder をアクティブ端末・プレビューペイン・検索バー入力欄へ）。キーは `NSEvent.keyEvent` で合成して `NSApp.postEvent` に積むので `sendEvent` → `MRUKeyMonitor` → responder chain と**本物と同じ経路**を通る。osascript の補助アクセス権限が不要なので PolePole 内の Claude Code からもキー操作を検証できる。処理結果は `[test-event]` ログで裏取りする（`focus` は 0.3 秒後の `settled` 行が確定値。SwiftUI `@FocusState` は非同期反映）。使用例は `scripts/verify-find-bar-focus.sh` |
...
- ライセンス API の向き先は Debug / Release とも既定で `https://polepole.dev`（`LicenseClient.defaultBaseURL()`）。ローカル wrangler dev でライセンスフローを検証するときだけ `POLEPOLE_BACKEND_URL=http://127.0.0.1:8787 ./scripts/polepole-launch.sh` のように明示上書きする
- **2026-09-09 まで Debug の既定は `127.0.0.1:8787` だった**。VERIFY のバイナリ直叩き・`open -n --env` 起動では `POLEPOLE_BACKEND_URL` が付かず、再検証が 30 日間一度も通らずに grace 切れでロックされる事故が 2 回起きた（2026-08-06 / 2026-09-09）ので既定を本番に変えた。ローカル backend が居ないときにアクティベートすると「ネットワークエラー」、既存 token の再検証は失敗し続けて**オフライン猶予（30日）切れでトライアル/購入モーダルが復活する**のは変わらない
...
- **`MRUKeyMonitor` で「オーバーレイやバーが表示中」だけを条件に Return / Esc を握らない**: 表示状態と first responder は別物で、検索バーが見えたままターミナルをクリックすると Return / Esc が横取りされ続けて端末が操作不能になった実例あり。非モーダルな UI のキーを握るときは `PreviewFocus.isFirstResponderWithinPreview()` のように first responder の位置（superview を辿ってホスト view に届くか）を併せて見る。SwiftUI `TextField` のフォーカス時の first responder は field editor（NSTextView）だが NSTextField の subview なので superview 経由で判定できる（[FilePreviewPaneView.swift](../Sources/polepole/FilePreviewPaneView.swift) / [PreviewFocusTests.swift](../polepoleTests/PreviewFocusTests.swift)）
- **起動時に `POLEPOLE_TEST_AUTO_PREVIEW` でプレビューを開く検証は、split の autosave をリセットしてから起動する**: UserDefaults の `NSSplitView Subview Frames polepole.rootSplit.v2` にプレビュー列が `collapsed=YES` で残っていると、起動時の復元で `isCollapsed` の KVO が発火 → `onPreviewCollapseDidChange(true)` → `preview.close()` が走り、AUTO_PREVIEW で開いたプレビュー（と検索バー）が 0.2 秒後に閉じる。実ユーザーは起動時にプレビューを開いていないので実害はないがテスト経路では毎回踏む。`defaults delete local.d0ne1s.polepole.dev "NSSplitView Subview Frames polepole.rootSplit.v2"` を起動前に入れる（`scripts/verify-find-bar-focus.sh` 参照）
- **XCTest で `NSWindow` を作るときは `isReleasedWhenClosed = false` にする**: 既定 true のままだと `close()` と ARC の解放で二重解放になり、テスト本体は通っているのに autorelease pool pop で `EXC_BAD_ACCESS`（`objc_release`）を起こして「Restarting after unexpected exit」で全テストが 0 件実行扱いになる。`defer { window.close() }` だけの既存テストは偶然通っているだけ
