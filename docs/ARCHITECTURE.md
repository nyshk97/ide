...
- `PreviewWebView`: WKWebView + highlight.js のラッパ。アプリ起動時に空 surface を pre-warm して初回プレビューの遅延を吸収。Markdown プレビュー内のリンクは `WKScriptMessageHandler`（weak ラッパで参照リーク回避）で横取りし、project root 配下のローカルリンクは `FilePreviewModel` で開き、外部 URL / project root 外の file はクリップボードコピーする。viewer.js は同じ判定でリンクに内部/外部アイコンを常時付ける。Markdown 中の `file://` 画像は `loadFileURL(_:allowingReadAccessTo:)` の読み取り権限（viewer.html のあるバンドル配下のみ）外で表示できないので、viewer.js が `<img src>` を `ideres://` に書き換え、`WKURLSchemeHandler`（`LocalResourceSchemeHandler`）が `allowedRoot` 配下チェック付きでディスクから読んで返す
