...
### FilePreviewModel / FilePreviewClassifier / FilePreviewView / PreviewWebView
...
- `FilePreviewView`: 種別ごとに表示を切替。コードは `PreviewWebView`、画像は `NSImage`、PDF は PDFKit、Markdown は WKWebView でレンダリング。ヘッダーはパンくず形式で `Cmd+J` でツリーとトグル
